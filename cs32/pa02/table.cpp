...
Table::Table( unsigned int max_entries ) { // an array of std::list
...
bool Table::remove(unsigned int key){
    ...
    if ( datatable[ index ].empty() ){
        ...
        for (list<Entry>::iterator it = datatable[ index ].begin(), end = datatable[ index ].end(); it != end; ++it){ 
                                            // checking each element of the list @index
            if ( it->get_key() == key ){
                datatable[ index ].erase( it );
                --numEntries;
                return true;
